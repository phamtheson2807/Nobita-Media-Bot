...
@media(max-width:1100px){.app{grid-template-columns:1fr}.side{display:none}.main{padding:20px}.mobile-brand{display:block;color:var(--cyan);font-size:10px;letter-spacing:2px}.metrics{grid-template-columns:repeat(2,1fr)}.metric:last-child{grid-column:1/-1}.grid{grid-template-columns:1fr}.topbar{align-items:flex-start}.chart{height:190px}}
...
document.querySelectorAll('.nav-btn').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.nav-btn').forEach(x=>x.classList.remove('on'));btn.classList.add('on');$(btn.dataset.target).scrollIntoView({behavior:'smooth'})}));
...
setInterval(()=>$('clock').textContent=new Date().toLocaleTimeString('vi-VN')+' ICT',1000);window.addEventListener('resize',()=>drawChart((state.stats||{}).platforms));load();setInterval(load,2000);
</script></body></html>`);
});
