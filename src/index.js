...
function switchView(target){const showAnalytics=target==='analytics'||target==='systems';$('overview').classList.toggle('view-hidden',target!=='overview');$('analytics').classList.toggle('view-hidden',!showAnalytics);$('tasks').classList.toggle('view-hidden',target!=='tasks');const chartPanel=$('analytics').children[0],systemPanel=$('systems');chartPanel.style.display=target==='systems'?'none':'';systemPanel.style.display=target==='analytics'?'none':'';$('analytics').style.gridTemplateColumns=showAnalytics?'1fr':'';document.querySelectorAll('.nav-btn').forEach(x=>x.classList.toggle('on',x.dataset.target===target));window.scrollTo({top:0,behavior:'smooth'});if(target==='analytics')setTimeout(()=>drawChart((state.stats||{}).platforms),60);sessionStorage.setItem('nobita-view',target)}
