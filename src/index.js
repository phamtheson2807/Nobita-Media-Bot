...
*{box-sizing:border-box}body{margin:0;font-family:Inter,ui-sans-serif,system-ui;background:#07111f;color:#eaf2ff}body:before{content:"";position:fixed;inset:0;background:radial-gradient(circle at 10% 0,#12345d66,transparent 35%),radial-gradient(circle at 90% 10%,#13504444,transparent 30%);pointer-events:none}.layout{display:grid;grid-template-columns:245px 1fr;min-height:100vh}.side{position:sticky;top:0;height:100vh;padding:28px 20px;border-right:1px solid #20334b;background:#091523dd;backdrop-filter:blur(18px)}.brand{font-size:20px;font-weight:800}.brand b{color:#5ee7ff}.nav{margin-top:32px}.nav div{padding:12px 14px;margin:7px 0;border-radius:11px;color:#8fa5bf}.nav .on{background:#15314b;color:#fff}.side-foot{position:absolute;bottom:25px;color:#7188a4;font-size:12px}.main{min-width:0;padding:28px 32px 60px}.header{display:flex;justify-content:space-between;gap:20px;align-items:center}.header h1{margin:0;font-size:28px}.sub{color:#88a0bb;margin-top:5px}.live{padding:8px 12px;border:1px solid #23523f;border-radius:99px;color:#65e6a5;background:#0e2b22}.cards{display:grid;grid-template-columns:repeat(5,minmax(145px,1fr));gap:14px;margin:24px 0}.card,.panel{border:1px solid #203650;background:linear-gradient(145deg,#0e1d30dd,#0a1727dd);box-shadow:0 14px 35px #0003;border-radius:16px}.card{padding:18px}.label{font-size:12px;color:#8299b4}.value{font-size:30px;font-weight:800;margin:6px 0}.delta{font-size:12px;color:#5fe5a0}.overview{display:grid;grid-template-columns:1.4fr .8fr;gap:14px}.panel{padding:20px}.panel h3{margin:0 0 17px}.platforms{display:grid;gap:13px}.p-row{display:grid;grid-template-columns:95px 1fr 38px;align-items:center;gap:10px}.track,.mini-track{height:8px;background:#1d3047;border-radius:9px;overflow:hidden}.track i,.mini-track i{display:block;height:100%;border-radius:9px;background:linear-gradient(90deg,#33b9e8,#6be5ff)}.health{display:flex;align-items:center;gap:22px}.ring{width:120px;height:120px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#5fe5a0 var(--rate),#203149 0);position:relative}.ring:after{content:"";position:absolute;inset:10px;border-radius:50%;background:#0c1a2b}.ring b{position:relative;z-index:1;font-size:25px}.health-list{display:grid;gap:9px;color:#9bb0c8}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:#5fe5a0;margin-right:7px}.tools{display:flex;gap:10px;flex-wrap:wrap;margin:22px 0 12px}.input,.select{background:#0d1c2e;border:1px solid #28405d;color:#eaf2ff;border-radius:11px;padding:11px 13px}.input{min-width:260px;flex:1}.select{min-width:150px}.table-wrap{overflow:auto;border:1px solid #203650;border-radius:16px;background:#0b1829}table{width:100%;border-collapse:collapse;min-width:1100px}th,td{text-align:left;padding:13px 14px;border-bottom:1px solid #192c43;font-size:13px}th{position:sticky;top:0;background:#102036;color:#839ab5;z-index:1}.badge{padding:5px 9px;border-radius:99px;background:#26364c}.badge.success{color:#6ce7aa;background:#123b2c}.badge.failed{color:#ffa3b2;background:#48212b}.badge.downloading,.badge.sending{color:#67dfff;background:#143b53}.url{max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#9cb3ce}.error{max-width:230px;color:#ffafba}.empty{text-align:center;padding:45px;color:#7890aa}.mobile-nav{display:none}@media(max-width:1050px){.layout{grid-template-columns:1fr}.side{display:none}.main{padding:20px}.cards{grid-template-columns:repeat(2,1fr)}.overview{grid-template-columns:1fr}.mobile-nav{display:block;color:#5ee7ff}.header h1{font-size:23px}}@media(max-width:520px){.cards{grid-template-columns:1fr 1fr}.card:last-child{grid-column:1/-1}.input{min-width:100%}}
</style></head><body><div class="layout"><aside class="side"><div class="brand">NOBITA <b>CONTROL</b></div><div class="nav"><div class="on">◈ Tổng quan</div><div>⇩ Tác vụ tải</div><div>◎ Người dùng</div><div>⚙ Hệ thống</div></div><div class="side-foot">Media Bot · Render</div></aside><main class="main"><header class="header"><div><div class="mobile-nav">NOBITA CONTROL</div><h1>Trung tâm điều hành</h1><div class="sub" id="updated">Đang kết nối dữ liệu...</div></div><div class="live">● Hệ thống hoạt động</div></header><section class="cards"><div class="card"><div class="label">TỔNG YÊU CẦU</div><div class="value" id="total">0</div><div class="delta">Tất cả nền tảng</div></div><div class="card"><div class="label">THÀNH CÔNG</div><div class="value" id="success">0</div><div class="delta" id="rateText">0% tỷ lệ</div></div><div class="card"><div class="label">THẤT BẠI</div><div class="value" id="failed">0</div><div class="delta" style="color:#ff9baa">Cần kiểm tra</div></div><div class="card"><div class="label">ĐANG XỬ LÝ</div><div class="value" id="active">0</div><div class="delta">Theo thời gian thực</div></div><div class="card"><div class="label">BỘ NHỚ</div><div class="value" id="memory">0</div><div class="delta">MB đang dùng</div></div></section><section class="overview"><div class="panel"><h3>Phân bổ nền tảng</h3><div class="platforms" id="platforms"></div></div><div class="panel"><h3>Sức khỏe hệ thống</h3><div class="health"><div class="ring" id="ring" style="--rate:0%"><b id="ringValue">0%</b></div><div class="health-list"><span><i class="dot"></i>Telegram Polling</span><span><i class="dot"></i>TikWMAPI: <b id="apiState">—</b></span><span><i class="dot"></i>Uptime: <b id="uptime">—</b></span></div></div></div></section><div class="tools"><input class="input" id="search" placeholder="Tìm theo người dùng, URL hoặc lỗi..."><select class="select" id="status"><option value="">Mọi trạng thái</option><option value="downloading">Đang tải</option><option value="sending">Đang gửi</option><option value="success">Thành công</option><option value="failed">Thất bại</option></select><select class="select" id="platform"><option value="">Mọi nền tảng</option><option>TikTok</option><option>Facebook</option><option>YouTube</option><option>Instagram</option></select></div><div class="table-wrap"><table><thead><tr><th>Trạng thái</th><th>Nền tảng</th><th>Người dùng</th><th>Tiến độ</th><th>Tốc độ</th><th>ETA</th><th>URL</th><th>Lỗi</th><th>Thời gian</th></tr></thead><tbody id="rows"></tbody></table><div class="empty" id="empty">Chưa có yêu cầu phù hợp</div></div></main></div><script>
const key=new URLSearchParams(location.search).get('key'),q=document.getElementById('search'),sf=document.getElementById('status'),pf=document.getElementById('platform');let state={jobs:[]};const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const duration=s=>{s=Math.floor(s);const d=Math.floor(s/86400),h=Math.floor(s%86400/3600),m=Math.floor(s%3600/60);return(d?d+'d ':'')+(h?h+'h ':'')+m+'m'};function render(){const s=state.stats||{},sys=state.system||{};total.textContent=s.total||0;success.textContent=s.success||0;failed.textContent=s.failed||0;active.textContent=s.active||0;memory.textContent=sys.memoryMB||0;rateText.textContent=(s.successRate||0)+'% tỷ lệ';ringValue.textContent=(s.successRate||0)+'%';ring.style.setProperty('--rate',(s.successRate||0)+'%');apiState.textContent=sys.tikwmapi?'Đã kết nối':'Chưa cấu hình';uptime.textContent=duration(s.uptime||0);const max=Math.max(1,...Object.values(s.platforms||{}));platforms.innerHTML=Object.entries(s.platforms||{}).map(([k,v])=>'<div class="p-row"><b>'+esc(k)+'</b><div class="track"><i style="width:'+(v/max*100)+'%"></i></div><span>'+v+'</span></div>').join('')||'<div class="sub">Chưa có dữ liệu</div>';const term=q.value.toLowerCase(),status=sf.value,platform=pf.value;const list=(state.jobs||[]).filter(j=>(!status||j.status===status)&&(!platform||j.platform===platform)&&(!term||JSON.stringify(j).toLowerCase().includes(term)));rows.innerHTML=list.map(j=>'<tr><td><span class="badge '+esc(j.status)+'">'+esc(j.status)+'</span></td><td>'+esc(j.platform)+'</td><td>'+esc(j.user)+'</td><td><div class="mini-track"><i style="width:'+Number(j.progress||0)+'%"></i></div> '+Number(j.progress||0).toFixed(1)+'%</td><td>'+esc(j.speed||'—')+'</td><td>'+esc(j.eta||'—')+'</td><td class="url" title="'+esc(j.url)+'">'+esc(j.url)+'</td><td class="error">'+esc(j.error||'—')+'</td><td>'+new Date(j.startedAt).toLocaleString('vi-VN')+'</td></tr>').join('');empty.style.display=list.length?'none':'block';updated.textContent='Cập nhật '+new Date().toLocaleTimeString('vi-VN')+' · tự động mỗi 2 giây'}async function load(){try{const r=await fetch('/api/admin/state?key='+encodeURIComponent(key),{cache:'no-store'});if(!r.ok)throw 0;state=await r.json();render()}catch{updated.textContent='Mất kết nối dữ liệu'}}[q,sf,pf].forEach(e=>e.addEventListener('input',render));load();setInterval(load,2000);
</script></body></html>`);
});
